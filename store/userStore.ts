...
export const useUserStore = create<UserState>((set) => ({
  ...
  fetchSupabaseProfile: async (userId) => {
    ...
    if (data) {
      set({
        ...
        loading: false,
        onboardingCompleted: !!data.display_name,
      });
    } else {
      set({ loading: false });
    }
        ...
        if (error && error.code === "PGRST116") {
          ...
          if (createError) {
            ...
            if (createError.code === "23505") {
              ...
              if (existingUser) {
                set({
                  supabaseProfile: existingUser,
                  onboardingCompleted: !!existingUser.display_name,
                });
                return existingUser;
              }
              ...
              if (fetchError) {
                console.error(
                  "Error fetching user after duplicate key error:",
                  fetchError
                );
                throw fetchError;
              }
          ...
          if (newUser) {
            set({
              supabaseProfile: newUser,
              onboardingCompleted: !!newUser.display_name,
            });
  ...
  updateUserBio: async (userId, bio) => {
    // Use service role client to bypass RLS for user profile updates
    const client = supabaseAdmin || supabase;
    const { data, error } = await client
      .from("users")
  ...
  clearSupabaseProfile: () =>
    set({ supabaseProfile: null, onboardingCompleted: false }),
